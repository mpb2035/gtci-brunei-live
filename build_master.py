...
for code, ind in indicators.items():
    ...
    # RESOLVED (2026-07): a full cross-check against the official GTCI 2025 Brunei
    # Darussalam ranking PDF confirmed that all_77_indicators.json / srcGroupData.json /
    # gtciData.json / educationData.json all agree with each other AND with the official
    # PDF on bn_rank, sg_score, sg_rank, and gap (100% match, incl. formula consistency
    # gap == bn_score - sg_score). ministry_source_mapping.json's values for these four
    # fields were the erroneous ones (it even fabricates ranks for indicators the PDF
    # marks "n/a"). The backbone's bn_rank/sg_score/sg_rank/gap are therefore overwritten
    # below with the verified all_77_indicators.json values. bn_score itself was already
    # correct in ministry_source_mapping.json (kept as-is), aside from one isolated typo
    # in the source data for 3.1.2 (20.55 -> 20.56 per the official PDF; fix at the source
    # in ministry_source_mapping.json directly if regenerating from scratch).
    for verified_field in ('bn_rank', 'sg_score', 'sg_rank', 'gap'):
        ind[verified_field] = a.get(verified_field)
...
with open('gtci_master_data.json', 'w', encoding='utf-8') as f:
    # indent=2 keeps the output human-readable (one field per line) instead of a single
    # multi-hundred-KB line, at the cost of a somewhat larger file size.
